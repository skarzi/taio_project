...
def associate_worker_with_project(flow_graph):
    ...
    for worker_id in range(task_info.workers_number):
        worker_label = labels_modifier.modify('workers', worker_id)
        for feature, edge_data in flow_graph.adj[worker_label].items():
            if edge_data['flow'] > 0:
                for project, edge_data in flow_graph.adj[feature].items():
                    if edge_data['flow'] > 0:
                        # TODO: decode project label to project_number
                        yield (worker_id, project)
...
def dev_solution(test_data_path):
    ...
    g = create_graph(task, labels_modifiers={
        'workers': lambda x: f'w_{x}',
        'features': lambda x: f'f_{x}',
        'projects': lambda x: f'p_{x}',
        'source': lambda x: 'S',
        'target': lambda x: 'T',
    })
